...
void RubiksCube::resolutionByLayers() {
    ...
    std::cout << "1. Make a cross on the top layer" << std::endl;
...
void RubiksCube::CrossOnTopLayer() {
    ...
    auto hasWhiteEdge = [&]() -> bool {
        for (auto &face : {'F', 'R', 'B', 'L', 'D'}) { // Parcourir toutes les faces sauf U
            for (int index : {1, 3, 5, 7}) { // Indices possibles pour les arêtes
                if (cubeState[face][index] == "W") {
                    std::cout << "Arete trouvee !!!!!!!!!" << "a face :" << face << "et index : " << index << std::endl;
    ...
    // Pour chaque arête blanche qui n'est pas sur la face jaune (U)
        for (auto &face : {'F', 'R', 'B', 'L', 'D'}) { // Exclure la face U car nous cherchons les arêtes non situées sur U
            for (int index : {1, 3, 5, 7}) { // Indices possibles pour les arêtes
                // Vérifier si l'arête à cet index est blanche
                if (cubeState[face][index] == "W") { // Si une arête blanche est trouvée
                        ...
                        if (face == 'D') {
                            ...
                            switch (index) {
                                case 1: parseMove("F2", true); break;
                                case 3: parseMove("L2", true); break;
                                case 5: parseMove("R2", true); break;
                                case 7: parseMove("B2", true); break;
                            }
                        } else {
                            switch (face) {
                                case 'F':
